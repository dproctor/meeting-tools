...
def main(arguments):
    ...
    parser.add_argument(
        "--dev_journal",
        help="Optional location of dev journal",
        type=str,
    )
    ...
    for meeting_id in os.listdir(args.notes_dir):
        ...
        for note in os.listdir(meeting_dir):
            if not re.search(r"^\d\d\d\d-\d\d-\d\d.txt$", note):
                continue
            if _process_file(args.notes_dir, meeting_id, note,
                             not printed_meeting_id, args):
                printed_meeting_id = True

    if args.dev_journal:
        _process_file("", args.dev_journal, "", True, args)


def _process_file(notes_dir, meeting_id, note, print_meeting_id, args):
    """ Returns True if meeting_id was printed, False otherwise."""
    if note:
        note_filename = os.path.join(notes_dir, meeting_id, note)
    else:
        note_filename = os.path.join(notes_dir, meeting_id)
    with open(note_filename, "r") as f:
        ...
        for line in f.readlines():
            if line == "\n" or (active_todo and _line_contains_todo(line)):
        ...
        if lines and _paragraph_contains_todo(lines):
            paragraphs.append({"lines": lines, "meeting_id": meeting_id})
        ...
        for paragraph in paragraphs:
            ...
            if args.owner is not None:
                if args.owner.lower() not in metadata["owners"]:
                    continue
            if args.inverse_owner is not None:
                if args.inverse_owner in metadata["owners"]:
                    continue
            if args.meeting_id is not None:
                if not re.search(args.meeting_id, paragraph["meeting_id"]
                                 ) and not note_filename == meeting_id:
                    # latter condition is for dev_journal
                    continue
        ...
        if print_meeting_id:
            print("\n" + bcolors.HEADER + bcolors.UNDERLINE + meeting_id +
                  bcolors.ENDC)
            ret_val = True
